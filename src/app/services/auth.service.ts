...
export class AuthService {
  ...
  isCompany() {
    ...
    return this.http
      .get('http://localhost:3000/api/isCompany', httpOptions)
  ...
  isUser() {
    ...
    return this.http
      .get('http://localhost:3000/api/isUser', httpOptions)
      .pipe(map((response: Response) => response));
  }
  ...
  signUpEmailUser(user) {
    return this.afAuth.auth
      ...
      .then(res => {
        ...
        this.signInEmail(user).then(nav => {
          this.loader.hide();
  ...
  signUpEmailCompany(company) {
    return this.afAuth.auth
      ...
      .then(res => {
        ...
        this.signInEmail(company).then(nav => {
          this.loader.hide();
          this.route.navigate(['/home']);
        });
      });
  ...
  signInWithGoogle() {
    return this.afAuth.auth
      ...
      .then(res => {
        this.loader.show();
        this.fireServ.getUser(res.user.uid).then(user => {
          if (user === undefined) {
